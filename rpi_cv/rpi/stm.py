...
class STMInterface:
    ...
    def send(self):
        ...
        while True:
            ...
            if message_type == "NAVIGATION":
                    ...
                    if message["data"]["commands"] != "SECONDLEFT" or message["data"]["commands"] != "SECONDRIGHT":
                        if idx >= len(commands) - NUM_IMAGES:
                            ...
                            capture_and_send_image_thread = threading.Thread(
                                target=self.send_image_to_pc(final_image=False), daemon=True)
                ...
                # Start a new thread to capture and send the image to PC
                capture_and_send_image_thread = threading.Thread(
                    target=self.send_image_to_pc(final_image=True), daemon=True)
                capture_and_send_image_thread.start()
                # temp code
                # message = {
                #     "type": 'test'
    ...
    def return_to_carpark(self):
        ...
        print(
            f"[STM] Initiating return to carpark: XDIST = {self.xdist}, YDIST = {self.ydist}, ARROW = {self.second_arrow}")
