...
# turn_wrt_big_turns = [[4 * TURN_RADIUS, 1 * TURN_RADIUS],
#                       [4 * TURN_RADIUS, 2 * TURN_RADIUS]]

turn_wrt_big_turns = [[4 * TURN_RADIUS, 2 * TURN_RADIUS],
                      [4 * TURN_RADIUS, 2 * TURN_RADIUS]]
            ...
            if md == direction:  # if the new direction == md
                ...
                if direction == Direction.NORTH and md == Direction.EAST:
                    ...
                    if self.grid.reachable(
                            x + bigger_change, y + smaller_change, turn=True) and self.grid.reachable(
                            x, y, preTurn=True):
                        # Get safe cost of destination
                    ...
                    # Check for valid position
                    if self.grid.reachable(
                            x - smaller_change, y - bigger_change, turn=True) and self.grid.reachable(
                            x, y, preTurn=True):
                        # Get safe cost of destination
